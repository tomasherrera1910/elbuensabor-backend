...
pedidosDetallesRouter.post('/', userExtractor, async (req, res, next) => {
  ...
  if (tipoDeArticulo === 'insumo') {
    ...
    if (bebida.stockActual < 1) { return res.status(400).json({ error: '¡Lo sentimos, no tenemos stock del producto indicado!' }) }
  }
  ...
  if (tipoDeArticulo === 'manufacturado') {
    ...
    for (const ingrediente of ingredientes) {
      ...
      if (ingredienteDB.cantidad > ingredienteInsumo.stockActual) { return res.status(400).json({ error: '¡Lo sentimos, no tenemos suficiente stock de alguno de los ingredientes del producto seleccionado!' }) }
    }
  }
...
pedidosDetallesRouter.put('/:id', userExtractor, async (req, res, next) => {
  ...
  if (tipoDeArticulo === 'insumo') {
    const bebida = await ArticuloInsumo.findById(articuloId)
    if (bebida.stockActual < cantidad) { return res.status(400).json({ error: `¡Lo sentimos, solo quedan ${(cantidad - 1)} unidades!` }) }
  }
  // VALIDAR STOCK DEL ARTICULO en caso de comida
  if (tipoDeArticulo === 'manufacturado') {
    ...
    for (const ingrediente of comida.ingredientes) {
      const ingredienteDB = await ArticuloManufacturadoDetalle.findById(ingrediente)
      const ingredienteInsumo = await ArticuloInsumo.findById(ingredienteDB.articuloInsumo)
      if ((ingredienteDB.cantidad * cantidad) > ingredienteInsumo.stockActual) { return res.status(400).json({ error: `¡Lo sentimos, el stock de ingredientes alcanza para ${(cantidad - 1)} unidades!` }) }
    }
  }
