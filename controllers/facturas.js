...
facturasRouter.post('/', async (req, res, next) => {
  ...
  const pedidoAsociado = await Pedido.findById(pedido)
  const totalPedido = pedidoAsociado.total
  const montoDescuento = pedidoAsociado.tipoEnvio === 'Retiro en el local' ? (((totalPedido * 100) / 90) * 0.10) : 0
