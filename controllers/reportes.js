        ...
        if (comida.tipoDeArticulo === 'manufacturado') { // Solo se tomaran en cuenta para el ranking articulos manufacturados
          ...
          for (const articulo of rankingComidas) { // Buscamos en el ranking si la comida ya fue agregada
            if (articulo.comida === comida.articulo) {
              const index = rankingComidas.findIndex(ranking => ranking.comida === articulo.comida)
          ...
          if (!articuloEncontrado) { rankingComidas = rankingComidas.concat({ comida: comida.articulo, cantidadPedida: comida.cantidad }) }
        }
      }
    }
  }
    ...
    if (datePedido >= dateDesde && datePedido <= dateHasta) {
      ...
      for (const usuario of rankingPedidosPorUsuario) {
        if (usuario.email === user.email) {
          const index = rankingPedidosPorUsuario.findIndex(ranking => ranking.email === usuario.email)
      ...
      // Si no fue agregada aún la agregamos
      if (!userEncontrado) { rankingPedidosPorUsuario = rankingPedidosPorUsuario.concat({ email: user.email, cantidadPedidos: 1 }) }
    }
  }
    ...
    if (datePedido >= dateDesde && datePedido <= dateHasta) {
      ...
      for (const fechaIngreso of fechasIngresos) {
        if (fechaIngreso.fecha === datePedido.toLocaleDateString()) {
          const index = fechasIngresos.findIndex(date => date.fecha === fechaIngreso.fecha)
      ...
      // Si no fue agregada aún la agregamos
      if (!fechaEncontrada) { fechasIngresos = fechasIngresos.concat({ fecha: datePedido.toLocaleDateString(), ingresos: pedido.total }) }
    }
  }
  ...
  for (const pedido of pedidos) {
    ...
    if (datePedido >= dateDesde && datePedido <= dateHasta) {
      ...
      for (const fechaIngreso of fechasGanancias) {
        if (fechaIngreso.fecha === datePedido.toLocaleDateString()) {
          const index = fechasGanancias.findIndex(date => date.fecha === fechaIngreso.fecha)
      ...
      // Si no fue agregada aún la agregamos
      if (!fechaEncontrada) { fechasGanancias = fechasGanancias.concat({ fecha: datePedido.toLocaleDateString(), ingresos: pedido.total }) }
    }
  }
  ...
  for (const insumo of insumos) {
    if (insumo.fecha) {
      ...
      if (dateInsumo >= dateDesde && dateInsumo <= dateHasta) {
        ...
        // Si no fue agregada aún la agregamos
        if (!fechaEncontrada) { fechasGanancias = fechasGanancias.concat({ fecha: dateInsumo.toLocaleDateString(), gastos: insumo.precioCompra }) }
      }
    }
  }
